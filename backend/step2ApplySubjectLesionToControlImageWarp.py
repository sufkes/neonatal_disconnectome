...
def applySubjectLesionToControlImageWarp(runs_dir, subject, lesion_image, age, skip = False):
  try:
    ...
    for d in dir_list:
      ...
      lesion_in_control_image_space = ants.apply_transforms(fixed=fixed_ants_img, moving=lesion_ants_img, transformlist=transformlist, verbose=False)
    ...
    lesion_warped_to_40_week = ants.apply_transforms(fixed=fixed_ants_img, moving=lesion_ants_img, transformlist=transformlist[1:],verbose=False)
